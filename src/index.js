...
function init() {
    ...
    var balloonTpl = ymaps.templateLayoutFactory.createClass(
        `<div class="opinion popover">
            <div class="opinion__header">
                <div class="opinion__marker"></div>
                <div class="opinion__adress">{{properties.data.address|default: address}}</div>
                <div class="btn-exit"></div>
            </div>
            <div class="opinion__body">
                <div class="opinion__comment-outer">{% include options.contentLayout %}</div>
                <div class="opinion__title">Ваш отзыв</div>
                <div class="input-wrap">
                    <input id="name" type="text" name="name" placeholder="Ваше имя">
                </div>
                <div class="input-wrap">
                    <input id="place" type="text" name="place" placeholder="Укажите место">
                </div>
                <div class="input-wrap">
                    <textarea id="comment" name="comment" cols="30" rows="5" placeholder="Поделитесь впечатлениями"></textarea>
                </div>
                <div class="btn__wrap">
        ...
        </div>`,
        {
            build() {
                ...
                var closeBtn = document.querySelector('.btn-exit'),
                    addComment = document.querySelector('.btn-add');
            ...
            addReview(e) {
                ...
                var name = document.getElementById('name'),
                    ...
                    reviews = document.querySelector('.opinion__comment-outer');
...
function clearInput(name, place, comment) {
    name.value = '';
    place.value = '';
    comment.value = '';
}